...
#    In der derzeitigen Version (v1.5) wird sich auf die Erstellung von Formelfragen beschränkt     #
#                                                                                                   #
#    Unterstützung von bis zu 10 Variablen und 10 Lösungen                                          #
#    Die Datenbank "ilias_questions_DB" muss ersetzt werden, da nun mehr Daten eingefügt werden     #
#    Pfad Anpassungen für Cross-Plattformen                                                         #
#    Option den Fragentext in Latex oder ohne Latex in die XML zu schreiben                         #
#                                                                                                   #
# ------------------------------------------------------------------------------------------------- #
...
class GuiMainWindow:
    ...
    def __init__(self, master):
        ...
        master.title('ilias - Test-Generator v1.5')


        # --------------------------    Set PATH for Project

        self.project_root_path = pathlib.Path().absolute()
        self.img_file_path_create_folder = os.path.normpath(os.path.join(self.project_root_path, '1590475954__0__tst_1944463', 'objects'))
        self.img_file_path = os.path.normpath(os.path.join(self.project_root_path, '1590475954__0__tst_1944463', 'objects'))
        ...
        self.tst_file_path_write = os.path.normpath(os.path.join(self.project_root_path, '1590475954__0__tst_1944463', '1590475954__0__tst_1944463.xml'))
        self.qti_file_path_write = os.path.normpath(os.path.join(self.project_root_path, '1590475954__0__tst_1944463', '1590475954__0__qti_1944463.xml'))
        ...
        self.qpl_file_pool_path_write = os.path.normpath(os.path.join(self.project_root_path, '1594724569__0__qpl_1950628', '1594724569__0__qpl_1950628.xml'))
        self.qti_file_pool_path_write = os.path.normpath(os.path.join(self.project_root_path, '1594724569__0__qpl_1950628', '1594724569__0__qti_1950628.xml'))


        # Taxonomy - Files
        #self.taxonomy_file_read = os.path.normpath(os.path.join(self.project_root_path, '1594724569__0__qpl_1950628', 'Services', 'Taxonomy', 'set_1', 'export.xml'))
        #self.taxonomy_file_write = os.path.normpath(os.path.join(self.project_root_path, '1594724569__0__qpl_1950628', 'Services', 'Taxonomy', 'set_1', 'export_edited.xml'))
        ...
        self.create_question_pool_btn = Button(self.frame_create_formelfrage, text="Create Question Pool", command=lambda: create_formelfrage_pool.__init__(self))
        ...
        # excel_export_btn
        self.excel_xlsx_export_btn = Button(self.frame_excel_sql, text="Export to Excel file",command=lambda: Database.sql_db_to_excel_export(self, self.table_name))
        ...
        # Read XML File
        self.read_XML_btn = Button(self.frame_latex_preview, text="Text \"READ XML\"",command=lambda: Formelfrage.read_XML(self))
        #self.read_XML_btn.grid(row=5, column=0, padx=10, sticky="W")
        ...
        self.create_question_pool_all_label.grid(row=4, column=0, pady=5, padx=5)
...
class Formelfrage(GuiMainWindow):
    ...
    def read_XML(self):
        ...
        # Create a ScrolledFrame widget
        self.sf_taxonomy = ScrolledFrame(self.taxonomy_window, width=self.settings_width, height=self.settings_height)
        ...
        print("Function: Read XML")
        self.mytree = ET.parse(self.qti_file_pool_path_write)
        ...
        for item in self.myroot.iter('item'):
            ...
            self.item_id = self.item_id_raw[-6:]
        ...
        for i in range(len(self.item_in_node)):
            print(str(self.item_in_node[i]) + " -------> " + str(self.id_to_node_dict.get(self.item_in_node[i])) + ", with Item ID: " + str(self.item_tag[i]) + "... with Array_index: " + str(self.item_nr_list[i]))

        for i in range(len(self.item_nr_list)):
            print("ID found in labels with Index: " + str(self.item_labels_list.index(self.item_tag[i])))
        ...
        print(self.check_question_id_start)
        print(self.check_question_id_end)

        for i in range(len(self.item_labels_list)):
            if int(self.item_labels_list[i]) != int(self.check_question_id_counter):
                print(self.check_question_id_counter)
                print("Error in Labels list")
        ...
        self.tax_nodes_myCombo = ttk.Combobox(self.taxonomy_frame_boxes, value=self.node_tag, width=30)
    ...
    def assign_questions_to_node(self):
        ...
        if self.entry_starting_id.get() != "" and self.entry_ending_id.get() != "":
            for i in range(int(self.entry_starting_id.get()), int(self.entry_ending_id.get())+1):
                for tax_node in self.myroot.iter('{http://www.ilias.de/Services/Taxonomy/tax/4_3}TaxNodeAssignment'):
        ...
        if self.node_to_id_dict.get(self.tax_nodes_myCombo.get()) != self.child_tag[0]:
            if self.entry_starting_id.get() != "" and self.entry_ending_id.get() != "":
                print(self.tax_nodes_myCombo.get())
                print(self.node_to_id_dict.get(self.tax_nodes_myCombo.get()))
                print(self.child_tag[0])

                for i in range(int(self.entry_starting_id.get()), int(self.entry_ending_id.get())+1):
                    # Die Definition der Haupt- und Sub-Elemente muss in der Schleife fr jede Frage neu erstellt werden
                    ...
                    print(ItemId.text + " with Node: " + NodeId.text + "... appended!")
            else:
        ...
        with open(self.taxonomy_file_write, 'r') as xml_file:
        ...
        with open(self.taxonomy_file_write, 'w') as replaced_xml_file:
    ...
    def remove_question_from_node(self):
        ...
        if self.entry_starting_id.get() != "" and self.entry_ending_id.get() != "":
            for i in range(int(self.entry_starting_id.get()), int(self.entry_ending_id.get()) + 1):
                for tax_node in self.myroot.iter('{http://www.ilias.de/Services/Taxonomy/tax/4_3}TaxNodeAssignment'):
                    if tax_node.find('{http://www.ilias.de/Services/Taxonomy/tax/4_3}ItemId').text == str(i):
                        self.myroot.delete(tax_node)
                        print("Delete Item-ID: " + str(i))
...
class Database(Formelfrage):
    ...
    def sql_db_to_excel_export(self, table_name):
        ...
        excel = xlsxwriter.Workbook(self.write_to_excel_path)
    ...
    def create_question_pool(self, x):
        ...
        for record in records:
            ...
            # -----------------------------------------------------------------------Variable 10
                ...
                print("Create Question_pool..DONE")
