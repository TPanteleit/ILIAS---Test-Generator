...
#    In der derzeitigen Version (v1.4) wird sich auf die Erstellung von Formelfragen beschränkt     #
#                                                                                                   #
#    Erstellung eines Fragenpools hinzugefügt                                                       #
#    Option zur Erstellung aller Fragen aus der DB in den Fragenpool                                #
#    Option zum entfernen aller Fragen aus der DB (hierbei wird zuvor ein Backup angelegt)          #
#                                                                                                   #
#    Hotfix für Erstellung von Variable $v4. Copy-Paste Fehler                                      #
#    "Beschreibung" von Fragen war auskommentiert                                                   #
#                                                                                                   #
# ------------------------------------------------------------------------------------------------- #
#    -----> WARNUNG: Wenn Bilder in der Datenbank hinterlegt werden,    <-----                      #
#    ----->          kann die DB nicht in Excel portiert werden         <-----                      #
#                                                                                                   #
#####################################################################################################
...
class GuiMainWindow:
    ...
    def __init__(self, master):
        ...
        master.title('ilias - Test-Generator v1.4')
        ...
        # Question Pool - Files
        self.qpl_file_pool_path_read = self.project_root_path + r"\orig_qpl_qti_files\orig_1594724569__0__qpl_1950628.xml"
        self.qti_file_pool_path_read = self.project_root_path + r"\orig_qpl_qti_files\orig_1594724569__0__qti_1950628.xml"

        self.qpl_file_pool_path_write = self.project_root_path + r"\1594724569__0__qpl_1950628\1594724569__0__qpl_1950628.xml"
        self.qti_file_pool_path_write = self.project_root_path + r"\1594724569__0__qpl_1950628\1594724569__0__qti_1950628.xml"
        ...
        self.frame_excel_sql.grid(row=9, column=0, padx=170, pady=10, sticky="SW")
        ...
        self.create_formelfrage_btn = Button(self.frame_create_formelfrage, text="Create Question from ID", command=lambda: create_formelfrage.__init__(self))
        self.create_formelfrage_btn.grid(row=0, column=0, sticky=W)

        self.create_pool_btn = Button(self.frame_create_formelfrage, text="Create Question Pool", command=lambda: create_formelfrage_pool.__init__(self))
        self.create_pool_btn.grid(row=2, column=0, sticky=W, pady=(10,0))
        ...
        self.database_submit_formelfrage_btn = Button(self.frame_database, text="Save as new ID", command=lambda: Database.submit(self))
        self.database_submit_formelfrage_btn.grid(row=2, column=0, sticky=W, pady=5)

        self.database_new_question_btn = Button(self.frame_database, text="New Question", command=lambda: Database.new_question(self))
        self.database_new_question_btn.grid(row=2, column=1, sticky=W, pady=5)

        self.database_edit_btn = Button(self.frame_database, text="Save", command=lambda: Database.edit(self))
        ...
        self.delete_all_label = Label(self.frame_database, text="Delete all DB entrys?")
        ...
        #excel_import_btn
        self.excel_xlsx_import_btn = Button(self.frame_excel_sql, text="Import to SQL Database", command=lambda: Database.excel_xlsx_import(self))
        self.excel_xlsx_import_btn.grid(row=0, column=1, sticky=W, pady=5, padx=10)

        # excel_export_btn
        self.excel_xlsx_export_btn = Button(self.frame_excel_sql, text="Export to Excel file",command=lambda: Database.sql_db_to_excel_export(self, "SQL_DB_export"))
        ...
        self.create_test_settings_label = Label(self.frame_create_formelfrage, text="Use Test-Settings?")
        self.create_test_settings_label.grid(row=1, column=0, pady=5, padx=5)
        ...
        self.create_question_pool_all_label = Label(self.frame_create_formelfrage, text="Create all question entrys?")
        self.create_question_pool_all_label.grid(row=3, column=0, pady=5, padx=5)
        ...
        self.create_question_pool_all.grid(row=3, column=1, sticky=W)
    ...
    def reallocate_text(self):
        ...
        for z in range(self.numbers_of_searchterm_italic):
            try:
                ...
                self.formula_question_entry.tag_add('ITALIC', self.search_italic1_begin, self.search_italic1_end + '+2c')

                #self.formula_question_entry.tag_config('CUR', offset=-4)
                ...
                self.search_index_start = self.search_italic1_end + '+2c'
...
class Database(Formelfrage):
    ...
    def load(self):
        ...
        for record in records:
            ...
            self.test_time_from_db = record[74]
            self.var_number = record[75]
            self.myCombo.set(record[75])
            self.res_number = record[76]
            self.myCombo_res.set(record[76])
    ...
    def delete(self):
        ...
        if self.var_delete_all.get() == 1:
            ...
            Database.sql_db_to_excel_export(self, "BACKUP_Export_from_SQL__" + actual_time)
    ...
    def sql_db_to_excel_export(self, table_name):
        ...
        workbook = xlsxwriter.Workbook(str(pathlib.Path().absolute()) + "\\" + self.table_name + '.xlsx')
        worksheet = workbook.add_worksheet('SQL - Database')
    ...
    def show_img_from_db(self):
        ...
        for record in records:
            ...
            if str(record[len(record) - 1]) == self.create_formelfrage_entry.get():
                ...
                self.rec_data = record[73]  #record[73] -> img_data_raw (as byte)
            ...
            for record in records:
                if str(record[len(record) - 1]) == self.entry_split[x]:
                    if record[2].lower() == "formelfrage":
                        ...
                        self.question_description_main = self.question_description_main_italic_start
                    ...
                    if record[2].lower() == "formelfrage":
                        ...
                        for i in range(1, len(self.question_description_main_raw)):
                            if self.question_description_main_raw[i] == '_':
                                ...
                                self.position_end = self.question_description_main_raw.find(" ", self.position_begin)
                                self.index_list.append(self.position_end)
                                self.question_description_main_raw = self.question_description_main_raw[
                                                                     :self.position_end] + ' </sub>' + self.question_description_main_raw[
                                                                                                      self.position_end:]
                        ...
                        for i in range(1, len(self.question_description_main_raw)):
                            if self.question_description_main_raw[i] == '^':
                                ...
                                self.index_list.append(self.position_end)
                                self.question_description_main_raw = self.question_description_main_raw[
                                                                     :self.position_end] + ' </sup>' + self.question_description_main_raw[
                                                                                                      self.position_end:]
                        ...
                        self.question_description_main_test = self.question_description_main_multi_replaced
                        self.question_description_main_latex_start = self.question_description_main_test.replace('\\)', " </span>")
                        self.question_description_main_latex_end = self.question_description_main_latex_start.replace(
                            '\\(', "<span class=\"latex\">")
                        self.question_description_main_sup = self.question_description_main_latex_end.replace('^', "<sup>")
                        # self.question_description_main_sup_end = self.question_description_main_sup_start.replace('</p>', "</sup>")
                        self.question_description_main_sub = self.question_description_main_sup.replace('_', "<sub>")
                        ...
                        print("Question Pool generated with Title --> \"" + self.question_title + "\"")
                        ...
                        print("Question type with 'multiple choice' found")
                        create_multiplechoice.create_mc_question(MultipleChoice, self.mytree, self.myroot,
                                                                 self.qti_file_path_read, self.qti_file_path_write,
                                                                 self.entry_split, x)
                # create_formelfrage.create_question(self, x)   LAST CHANGE
        conn.commit()
        conn.close()
    ...
    def create_question_pool(self, x):
        ...
        for record in records:
            ...
            if str(record[len(record) - 1]) == self.entry_split[x]:
                ...
                if self.img_data_raw != "EMPTY":
                    # img wird immer als PNG Datei abgelegt.
                    with open(self.img_file_path + "\\il_0_mob_000000" + str(x) + "\\" + self.img_name + ".png",
                              'wb') as image_file:
                        image_file.write(self.img_data_raw)

                    self.image = Image.open(
                        self.img_file_path + "\\il_0_mob_000000" + str(x) + "\\" + self.img_name + ".png")
                    self.image.save(self.img_file_path + "\\il_0_mob_000000" + str(x) + "\\" + self.img_name + ".png")
                ...
                if self.img_data != "EMPTY":
                    # mattext.text = "<p>" + self.question_description_main + "</p>" + "<p><img height=\"378\" src=\"il_0_mob_0000000\" width=\"482\" /></p>"
                    mattext.text = "<p>" + self.question_description_main + "</p>" + "<p><img height=\"378\" src=\"il_0_mob_000000" + str(
                        x) + "\" width=\"482\" /></p>"
                                      ...
                                      "s:9:\"precision\";i:" + self.var1_prec + ";" \
                                                                                "s:12:\"intprecision\";s:" + self.var1_divby_length + ":\"" + self.var1_divby + "\";" \
                                                                                                                                                                "s:8:\"rangemin\";d:" + self.var1_min + ";" \
                                                                                                                                                                                                        "s:8:\"rangemax\";d:" + self.var1_max + ";" \
                                                                                                                                                                                                                                                "s:4:\"unit\";s:" + str(
                        self.var1_unit_length) + ":\"" + self.var1_unit + "\";" \
                                                                          "s:9:\"unitvalue\";s:" + str(
                        len(Formelfrage.unit_table(self, self.var1_unit))) + ":\"" + Formelfrage.unit_table(self,
                                                                                                            self.var1_unit) + "\";" \
                                                                                                                              "}"
                else:
                    fieldentry.text = "a:6:{" \
                                      ...
                                      "s:9:\"precision\";i:" + self.var2_prec + ";" \
                                                                                "s:12:\"intprecision\";s:" + self.var2_divby_length + ":\"" + self.var2_divby + "\";" \
                                                                                                                                                                "s:8:\"rangemin\";d:" + self.var2_min + ";" \
                                                                                                                                                                                                        "s:8:\"rangemax\";d:" + self.var2_max + ";" \
                                                                                                                                                                                                                                                "s:4:\"unit\";s:" + str(
                        self.var2_unit_length) + ":\"" + self.var2_unit + "\";" \
                                                                          "s:9:\"unitvalue\";s:" + str(
                        len(Formelfrage.unit_table(self, self.var2_unit))) + ":\"" + Formelfrage.unit_table(self,
                                                                                                            self.var2_unit) + "\";" \
                                                                                                                              "}"
                else:
                    fieldentry.text = "a:6:{" \
                                      ...
                                      "s:9:\"precision\";i:" + self.var3_prec + ";" \
                                                                                "s:12:\"intprecision\";s:" + self.var3_divby_length + ":\"" + self.var3_divby + "\";" \
                                                                                                                                                                "s:8:\"rangemin\";d:" + self.var3_min + ";" \
                                                                                                                                                                                                        "s:8:\"rangemax\";d:" + self.var3_max + ";" \
                                                                                                                                                                                                                                                "s:4:\"unit\";s:" + str(
                        self.var3_unit_length) + ":\"" + self.var3_unit + "\";" \
                                                                          "s:9:\"unitvalue\";s:" + str(
                        len(Formelfrage.unit_table(self, self.var3_unit))) + ":\"" + Formelfrage.unit_table(self,
                                                                                                            self.var3_unit) + "\";" \
                                                                                                                              "}"
                else:
                    fieldentry.text = "a:6:{" \
                                      ...
                                      "s:9:\"precision\";i:" + self.var4_prec + ";" \
                                                                                "s:12:\"intprecision\";s:" + self.var4_divby_length + ":\"" + self.var4_divby + "\";" \
                                                                                                                                                                "s:8:\"rangemin\";d:" + self.var4_min + ";" \
                                                                                                                                                                                                        "s:8:\"rangemax\";d:" + self.var4_max + ";" \
                                                                                                                                                                                                                                                "s:4:\"unit\";s:" + str(
                        self.var4_unit_length) + ":\"" + self.var4_unit + "\";" \
                                                                          "s:9:\"unitvalue\";s:" + str(
                        len(Formelfrage.unit_table(self, self.var4_unit))) + ":\"" + Formelfrage.unit_table(self,
                                                                                                            self.var4_unit) + "\";" \
                                                                                                                              "}"
                else:
                    fieldentry.text = "a:6:{" \
                                      ...
                                      "s:9:\"precision\";i:" + self.var5_prec + ";" \
                                                                                "s:12:\"intprecision\";s:" + self.var5_divby_length + ":\"" + self.var5_divby + "\";" \
                                                                                                                                                                "s:8:\"rangemin\";d:" + self.var5_min + ";" \
                                                                                                                                                                                                        "s:8:\"rangemax\";d:" + self.var5_max + ";" \
                                                                                                                                                                                                                                                "s:4:\"unit\";s:" + str(
                        self.var5_unit_length) + ":\"" + self.var5_unit + "\";" \
                                                                          "s:9:\"unitvalue\";s:" + str(
                        len(Formelfrage.unit_table(self, self.var5_unit))) + ":\"" + Formelfrage.unit_table(self,
                                                                                                            self.var5_unit) + "\";" \
                                                                                                                              "}"
                                      ...
                                      "s:9:\"precision\";i:" + self.var6_prec + ";" \
                                                                                "s:12:\"intprecision\";s:" + self.var6_divby_length + ":\"" + self.var6_divby + "\";" \
                                                                                                                                                                "s:8:\"rangemin\";d:" + self.var6_min + ";" \
                                                                                                                                                                                                        "s:8:\"rangemax\";d:" + self.var6_max + ";" \
                                                                                                                                                                                                                                                "s:4:\"unit\";s:" + str(
                        self.var6_unit_length) + ":\"" + self.var6_unit + "\";" \
                                                                          "s:9:\"unitvalue\";s:" + str(
                        len(Formelfrage.unit_table(self, self.var6_unit))) + ":\"" + Formelfrage.unit_table(self,
                                                                                                            self.var6_unit) + "\";" \
                                                                                                                              "}"
                                      ...
                                      "s:9:\"precision\";i:" + self.var7_prec + ";" \
                                                                                "s:12:\"intprecision\";s:" + self.var7_divby_length + ":\"" + self.var7_divby + "\";" \
                                                                                                                                                                "s:8:\"rangemin\";d:" + self.var7_min + ";" \
                                                                                                                                                                                                        "s:8:\"rangemax\";d:" + self.var7_max + ";" \
                                                                                                                                                                                                                                                "s:4:\"unit\";s:" + str(
                        self.var7_unit_length) + ":\"" + self.var7_unit + "\";" \
                                                                          "s:9:\"unitvalue\";s:" + str(
                        len(Formelfrage.unit_table(self, self.var7_unit))) + ":\"" + Formelfrage.unit_table(self,
                                                                                                            self.var7_unit) + "\";" \
                                                                                                                              "}"
                                                                                                                                                                                                                               ...
                                                                                                                                                                                                                               "s:8:\"rangemax\";s:" + self.res1_max_length + ":\"" + self.res1_max + "\";" \
                                                                                                                                                                                                                                                                                                      "s:6:\"points\";s:1:\"" + self.res1_points + "\";" \
                                                                                                                                                                                                                                                                                                                                                   "s:7:\"formula\";s:" + self.res1_formula_length + ":\"" + self.res1_formula + "\";" \
                                                                                                                                                                                                                                                                                                                                                                                                                                 "s:6:\"rating\";s:0:\"\";" \
                                                                                                                                                                                                                                                                                                                                                                                                                                 "s:4:\"unit\";s:" + str(
                        self.res1_unit_length) + ":\"" + self.res1_unit + "\";" \
                                                                          "s:9:\"unitvalue\";s:" + str(
                        len(Formelfrage.unit_table(self, self.res1_unit))) + ":\"" + Formelfrage.unit_table(self,
                                                                                                            self.res1_unit) + "\";" \
                                                                                                                              "s:11:\"resultunits\";a:27:{i:0;a:2:{s:4:\"unit\";s:1:\"H\";s:9:\"unitvalue\";s:3:\"125\";}" \
                                                                                                                              "i:1;a:2:{s:4:\"unit\";s:2:\"mH\";s:9:\"unitvalue\";s:3:\"126\";}" \
                                                                                                                              "i:2;a:2:{s:4:\"unit\";s:3:\"µH\";s:9:\"unitvalue\";s:3:\"127\";}" \
                                                                                                                                                                                                                               ...
                                                                                                                                                                                                                               "s:8:\"rangemax\";s:" + self.res2_max_length + ":\"" + self.res2_max + "\";" \
                                                                                                                                                                                                                                                                                                      "s:6:\"points\";s:1:\"" + self.res2_points + "\";" \
                                                                                                                                                                                                                                                                                                                                                   "s:7:\"formula\";s:" + self.res2_formula_length + ":\"" + self.res2_formula + "\";" \
                                                                                                                                                                                                                                                                                                                                                                                                                                 "s:6:\"rating\";s:0:\"\";" \
                                                                                                                                                                                                                                                                                                                                                                                                                                 "s:4:\"unit\";s:" + str(
                        self.res2_unit_length) + ":\"" + self.res2_unit + "\";" \
                                                                          "s:9:\"unitvalue\";s:" + str(
                        len(Formelfrage.unit_table(self, self.res2_unit))) + ":\"" + Formelfrage.unit_table(self,
                                                                                                            self.res2_unit) + "\";" \
                                                                                                                              "s:11:\"resultunits\";a:27:{i:0;a:2:{s:4:\"unit\";s:1:\"H\";s:9:\"unitvalue\";s:3:\"125\";}" \
                                                                                                                              "i:1;a:2:{s:4:\"unit\";s:2:\"mH\";s:9:\"unitvalue\";s:3:\"126\";}" \
                                                                                                                              "i:2;a:2:{s:4:\"unit\";s:3:\"µH\";s:9:\"unitvalue\";s:3:\"127\";}" \
                                                                                                                                                                                                                               ...
                                                                                                                                                                                                                               "s:8:\"rangemax\";s:" + self.res3_max_length + ":\"" + self.res3_max + "\";" \
                                                                                                                                                                                                                                                                                                      "s:6:\"points\";s:1:\"" + self.res3_points + "\";" \
                                                                                                                                                                                                                                                                                                                                                   "s:7:\"formula\";s:" + self.res3_formula_length + ":\"" + self.res3_formula + "\";" \
                                                                                                                                                                                                                                                                                                                                                                                                                                 "s:6:\"rating\";s:0:\"\";" \
                                                                                                                                                                                                                                                                                                                                                                                                                                 "s:4:\"unit\";s:" + str(
                        self.res3_unit_length) + ":\"" + self.res3_unit + "\";" \
                                                                          "s:9:\"unitvalue\";s:" + str(
                        len(Formelfrage.unit_table(self, self.res3_unit))) + ":\"" + Formelfrage.unit_table(self,
                                                                                                            self.res3_unit) + "\";" \
                                                                                                                              "s:11:\"resultunits\";a:27:{i:0;a:2:{s:4:\"unit\";s:1:\"H\";s:9:\"unitvalue\";s:3:\"125\";}" \
                                                                                                                              "i:1;a:2:{s:4:\"unit\";s:2:\"mH\";s:9:\"unitvalue\";s:3:\"126\";}" \
                                                                                                                              "i:2;a:2:{s:4:\"unit\";s:3:\"µH\";s:9:\"unitvalue\";s:3:\"127\";}" \
