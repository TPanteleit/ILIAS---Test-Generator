...
import test_generator_modul_datenbanken_erstellen
import test_generator_modul_ilias_test_struktur
import test_generator_modul_taxonomie_und_textformatierung
import test_generator_modul_ilias_import_test_datei
...
###################### "DATENBANK ENTRIES UND INDEX DICT  ###################
        ...
        # Auslesen der Zuordnungsfrage-Datenbank einträgen
        # Nur die erste Zeile auslesen um einen Zusammenhang zwimqhen Variablen und Indexen herzustellen
        self.mq_db_find_entries = []
        self.mq_db_find_indexes = []

        connect = sqlite3.connect('ilias_zuordnungsfrage_db.db')
        cursor = connect.cursor()
        cursor.execute("SELECT * FROM zuordnungsfrage_table LIMIT 1")

        mq_db_records = cursor.fetchall()
        for mq_db_record in mq_db_records:
            for k in range(len(mq_db_record)):
                self.mq_db_find_entries.append(str(mq_db_record[k]))
                self.mq_db_find_indexes.append(int(k))


        self.mq_db_entry_to_index_dict = dict(zip((self.mq_db_find_entries), (self.mq_db_find_indexes)))
...
###################### "Zuordnungsfrage-Datenbank" - FRAME   -------- LABELS / ENTRYS / BUTTONS  ###################
        ...
        self.mq_database_show_db_zuordnungsfrage_btn = Button(self.mq_frame_database, text="MQ - Datenbank anzeigen", command=lambda: test_generator_modul_datenbanken_anzeigen.MainGUI.__init__(self, "ilias_zuordnungsfrage_db", "zuordnungsfrage_table"))
...
###################### "Excel Import/Export" - FRAME   -------- LABELS / ENTRYS / BUTTONS  ###################
        ...
        # excel_export_btn
        self.mq_excel_export_to_xlsx_zuordnungsfrage_btn = Button(self.mq_frame_excel_import_export, text="Datenbank exportieren",command=lambda: test_generator_modul_datenbanken_erstellen.Import_Export_Database.excel_export_to_xlsx(self, self.project_root_path, self.mq_db_entry_to_index_dict, "ilias_zuordnungsfrage_db.db", "zuordnungsfrage_table", "Zuordnungsfrage_DB_export_file.xlsx", "Zuordnungsfrage - Database"))
        self.mq_excel_export_to_xlsx_zuordnungsfrage_btn.grid(row=1, column=1, sticky=W, pady=5, padx=10)

        # ILIAS_testfile_import
        self.mq_import_ilias_testfile_btn = Button(self.mq_frame_excel_import_export, text="ILIAS-Datei importieren",command=lambda: test_generator_modul_ilias_import_test_datei.Import_ILIAS_Datei_in_DB.__init__(self))
...
################
    ...
    def mq_save_id_to_db(self):
        conn = sqlite3.connect('ilias_zuordnungsfrage_db.db')
        c =conn.cursor()
        ...
        print("Neue Frage in DB gespeichert. --> Titel: " + str(self.mq_question_title_entry.get()))
    ...
    def mq_delete_id_from_db(self):
        ...
        test_generator_modul_datenbanken_erstellen.Delete_Entry_from_Database.__init__(self, self.mq_delete_box_id, "zuordnungsfrage", self.mq_var_delete_all.get(), self.project_root_path, self.mq_db_entry_to_index_dict, "ilias_zuordnungsfrage_db.db", "zuordnungsfrage_table", "Zuordnungsfrage_DB_export_file.xlsx", "Zuordnungsfrage - Database")
...
class Create_Zuordnungsfrage_Questions(Zuordnungsfrage):
    def __init__(self, db_entry_to_index_dict, ids_in_entry_box, question_type_test_or_pool, pool_img_dir, ilias_id_pool_qpl_dir, xml_read_qti_template_path, xml_qti_output_file_path, xml_qpl_output_file_path, max_id_pool_qti_xml, max_id, taxonomy_file_question_pool):
        ...
        connect_mq_db = sqlite3.connect('ilias_zuordnungsfrage_db.db')
    ...
    def mq_question_structure(self, id_nr):
        ...
        mq_connect = sqlite3.connect('ilias_zuordnungsfrage_db.db')
